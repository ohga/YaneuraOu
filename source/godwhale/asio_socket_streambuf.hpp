...
#ifndef ASIO_SOCKET_STREAMBUF_HPP
#define ASIO_SOCKET_STREAMBUF_HPP
...
/// boost::asio::basic_socket_streamにバグがあるようなので泣く泣く作成
/**
 * windows版のboost1.59.0にはunderflowでデータを二重に取得してしまうという
 * バグがあるので、その部分を修正したクラスを用意した。
 */
...
public:
    ...
    Time, TimeTraits, TimerService>* connect(const std::string &host, const std::string &service)
  {
    ...
    resolver_query query(host, service);    
    ...
    if (!ec_)
    {
        ...
        while (ec_ && i != end)
        {
            ...
            this->basic_socket<Protocol, StreamSocketService>::connect(*i, ec_);
            ++i;
        }
    }
...
protected:
  int_type underflow()
  {
    if (gptr() == egptr())
    {
      ...
      // 修正
      bytes_transferred_ = this->get_service().receive(
  ...
  int_type overflow(int_type c)
  {
    if (unbuffered_)
    {
      ...
      else
      {
        ...
        bytes_transferred_ = this->get_service().send(
            this->get_implementation(),
            boost::asio::buffer(&ch, sizeof(char_type)), 0, ec_);
        if (ec_)
          return traits_type::eof();
    ...
    else
    {
      ...
      while (boost::asio::buffer_size(buffer) > 0)
      {
        ...
        bytes_transferred_ = this->get_service().send(
            this->get_implementation(),
            boost::asio::buffer(buffer), 0, ec_);
        if (ec_)
          return traits_type::eof();
